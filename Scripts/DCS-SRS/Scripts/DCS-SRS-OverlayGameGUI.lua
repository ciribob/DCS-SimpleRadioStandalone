...
log.write('SRS-OverlayGameGUI', log.INFO, "Loading - DCS-SRS Overlay GameGUI - Ciribob: 2.3.0.3 ")
...
function srsOverlay.updateRadio()
    ...
    if _radioState and _radioState.RadioInfo and _radioState.RadioInfo.radios then
        ...
        for _i, _radio in pairs(_radioInfo.radios) do
            ...
            local transmitting = _radioState.RadioSendingState
                and _radioState.RadioSendingState.IsSending
                and (_radioState.RadioSendingState.SendingOn == _i - 1 or _radio.simul)
...
function srsOverlay.isTransmitting(_radioPos, _radio)
    return _radioState.RadioSendingState
        and _radioState.RadioSendingState.IsSending
        and (_radioState.RadioSendingState.SendingOn + 1 == _radioPos or _radio.simul)
end
...
function srsOverlay.paintRadio()
    ...
    if #_listMessages == 0 then
        if enabled then
            ...
            table.insert(
                _listMessages,
                { message = "Connect to an SRS server and join a mission", skin = typesMessage.guard, height = 20 }
            )
            table.insert(_listMessages, { message = "SRS DCS settings:", skin = typesMessage.guard, height = 20 })
            table.insert(
                _listMessages,
                { message = "Options -> SPECIAL -> DCS-SRS", skin = typesMessage.guard, height = 20 }
            )
            table.insert(
                _listMessages,
                { message = "Toggle (by default) with:", skin = typesMessage.guard, height = 20 }
            )
            table.insert(
                _listMessages,
                { message = "LEFT CTRL + LEFT SHIFT + ESC", skin = typesMessage.guard, height = 20 }
            )
        else
            table.insert(_listMessages, { message = "SRS not connected", skin = typesMessage.guard, height = 20 })
        end
...
function srsOverlay.createWindow()
    window =
        DialogLoader.spawnDialogFromFile(lfs.writedir() .. "Mods\\Services\\DCS-SRS\\UI\\DCS-SRS-Overlay.dlg", cdata)
...
function srsOverlay.setMode(mode)
    ...
    if srsOverlay.config.mode == _modes.hidden then
        ...
        if
            srsOverlay.config.mode == _modes.minimum
            or srsOverlay.config.mode == _modes.minimum_vol
            or srsOverlay.config.mode == _modes.txrx_only
        then
...
net.log("Loaded - DCS-SRS Overlay GameGUI - Ciribob: 2.3.0.3 ")
