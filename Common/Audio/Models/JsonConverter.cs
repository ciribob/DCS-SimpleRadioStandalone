...
namespace Ciribob.DCS.SimpleRadio.Standalone.Common.Audio.Models
{
    namespace Dto
    {
        internal class JsonIFilterConverter : JsonConverter<IFilter>
        {
                ...
                while (reader.Read())
                {
                    if (reader.TokenType == JsonTokenType.EndObject)
                    {
                        ...
                        if (q.HasValue)
                        {
                            ...
                            {
                                case "lowpass":
                                    return new BiQuadLowPassFilter
                                    {
                                        Frequency = frequency.Value,
                                        Q = q.Value
                                    };
                                case "highpass":
                                    return new BiQuadHighPassFilter
                                    {
                                        Frequency = frequency.Value,
                                        Q = q.Value
                                    };
                                case "peak":
                                    ...
                                    return new BiQuadPeakingEQFilter
                                    {
                                        Frequency = frequency.Value,
                                        Q = q.Value,
                                        Gain = gain.Value
